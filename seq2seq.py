...
def embedding_attention_seq2seq(encoder_inputs,
                                decoder_inputs,
                                #cell,    #20171018 comment out
                                enc_cell, #20171018 add
                                dec_cell, #20171018 add
                                num_encoder_symbols,
                                num_decoder_symbols,
                                embedding_size,
                                num_heads=1,
  ...
  with variable_scope.variable_scope(
    ...
    dtype = scope.dtype
    # Encoder.
    #encoder_cell = copy.deepcopy(cell) #20171018 comment out
    encoder_cell = enc_cell             #20171018 add
    encoder_cell = core_rnn_cell.EmbeddingWrapper(
        encoder_cell,
        embedding_classes=num_encoder_symbols,
        embedding_size=embedding_size)
    ...
    top_states = [
        #array_ops.reshape(e, [-1, 1, cell.output_size]) for e in encoder_outputs         #20171018 comment out
        array_ops.reshape(e, [-1, 1, encoder_cell.output_size]) for e in encoder_outputs  #20171018 add
    ]
    ...
    output_size = None
    if output_projection is None:
      #cell = core_rnn_cell.OutputProjectionWrapper(cell, num_decoder_symbols)         #20171018 comment out
      dec_cell = core_rnn_cell.OutputProjectionWrapper(dec_cell, num_decoder_symbols)  #20171018 add
