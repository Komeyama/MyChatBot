...
tf.app.flags.DEFINE_integer("from_vocab_size",1000, "English vocabulary size.") #40000
tf.app.flags.DEFINE_integer("to_vocab_size", 1000, "French vocabulary size.")    #40000
tf.app.flags.DEFINE_string("data_dir", "bot/datas", "Data directory")
tf.app.flags.DEFINE_string("train_dir", "bot/datas", "Training directory.")
...
def decode(input_text,sess,model):    #引数にsess,modelを追加
  #with tf.Session() as sess:          #2017/12/11コメントアウト
    ...
    out_vocab_path = os.path.join("bot/datas","vocab_out.txt" )

    in_vocab, _ = data_utils.initialize_vocabulary(in_vocab_path)
    _, rev_out_vocab = data_utils.initialize_vocabulary(out_vocab_path)

    #sentenceTemp = sys.stdin.readline()  #ターミナルで動かすとき用
    sentenceTemp = input_text
    tagger = MeCab.Tagger("-Owakati")
    sentence = tagger.parse(sentenceTemp)
    ...
    _1, _2, output_logits = model.step(sess, encoder_inputs, decoder_inputs,
                                       target_weights, bucket_id, True)
    ...
    output_text = " "
    output_text = " ".join([rev_out_vocab[output] for output in outputs])
